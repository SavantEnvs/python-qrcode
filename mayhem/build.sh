...
#    pillow are the optional renderers the test suite exercises. At this historical commit the
#    project still ships setup.py/setup.cfg (pre-poetry), so the no-build-isolation editable
#    install needs setuptools importable on PYTHONPATH (py3.13 no longer bundles it).
PKGS=(atheris pytest deprecation pypng "pillow>=9.1.0" "poetry-core>=2" "setuptools<81" typing_extensions)
need_download=0
