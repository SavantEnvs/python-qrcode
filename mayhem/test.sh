...
#
# Two deselects: both are historical test bugs exposed by a modern unittest.mock/Pillow, not
# by anything under mayhem/ or by the qrcode encoder logic this target fuzzes.
#   - test_release.py::test_change calls `mock_file().write.has_calls(...)` (a typo for
#     `assert_has_calls`); mock has raised AttributeError on unknown assert-like names since
#     Python 3.8, so this always-a-no-op assertion now errors instead of silently passing.
#   - test_script.py::test_piped mocks sys.stdout and lets Pillow .save() write to
#     mock_stdout.buffer; modern MagicMock auto-implements __fspath__, so Pillow now treats the
#     mock as a path-like and tries to open() its str() repr as a filename.
LOG="$(mktemp)"
"$RUNNER" -p no:cacheprovider -o addopts= -q qrcode/tests \
  --deselect qrcode/tests/test_release.py::UpdateManpageTests::test_change \
  --deselect qrcode/tests/test_script.py::ScriptTest::test_piped 2>&1 | tee "$LOG"
rc=${PIPESTATUS[0]}
